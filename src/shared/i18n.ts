...
const EN: Dict = {
  ...
  "privacy.close": "Close",
  "privacy.body": "This extension sends page content to the JiuwenSwarm server you configure (default ws://127.0.0.1). If that server runs on your machine, content stays local; if you point the extension at a remote server, content is sent there instead. The extension itself contacts no other service. Note: your JiuwenSwarm server forwards requests to whatever LLM provider you have configured (a local model or a cloud API), which happens between your server and the LLM, outside this extension's control. Page content is also held in the extension's cache and Chrome local storage. Sessions are stored on your JiuwenSwarm server. Uninstalling the extension deletes all locally stored data.",
...
const ZH: Dict = {
  ...
  "privacy.title": "隐私",
  "privacy.close": "关闭",
  "privacy.body": "此扩展会将页面内容发送到你配置的 JiuwenSwarm 服务器（默认为 ws://127.0.0.1）。如果该服务器运行在你本机，内容将保留在本地；如果你将扩展指向远程服务器，则内容会发送到该服务器。扩展本身不会联系其他任何服务。请注意：你的 JiuwenSwarm 服务器会把请求转发给你配置的大模型提供商（本地模型或云端 API），这一步发生在你的服务器与 LLM 之间，不受此扩展控制。页面内容也保存在扩展的缓存和 Chrome 本地存储中。会话存储在你的 JiuwenSwarm 服务器上。卸载扩展会删除所有本地存储的数据。",
